...
cc.Class({
    ...
    onOpenView: function (msg) {
        cc.log(msg);
        //var bg = this.node.getChildByName('bg');
        //bg.scale = 1;
        this.view_arr[0].active = true;
        this.view_arr[1].active = false;
        if("dai" == msg) this.sendDaiList();
        else this.sendZhanjiList();
    ...
    refPage:function(str){
        let lab,max_num;
        if(this.view_arr[1].active){
            lab = cc.find("btn_pagination/label",this.view_arr[1]).getComponent(cc.Label);
            max_num = this.page_1
        }else{
            lab = cc.find("btn_pagination/label",this.view_arr[0]).getComponent(cc.Label);
            max_num = this.page_0
        }
    ...
    updateView: function (num) {
        let arr, content, len, item_ind, index;
        if(this.view_arr[1].active){
            arr = this.list_data_1;
            len = arr.data.length;
            content = this.content_arr[1];
            index = 1;
            item_ind = this.item_arr[1]
        }else{
            arr = this.list_data_0;
            len = arr.data.length;
            content = this.content_arr[0];
            index = 0;
            item_ind = this.item_arr[0]
        }
        ...
        for (let i = num * 10; i < number; ++i) {
            ...
            if (len && i < len) {
                if (j < child_len) {
                    ...
                    item = cc.instantiate(item_ind);
                    item.parent = content;
                }
                this.showItem(item, arr.data[i], i + 1, index);
            } else {
                if (j < child_len) {
                    item = child_arr[j];
                    item.active = false;
     ...
     */
    updateViewO: function (data, index) {
        this.initView(index);
        let arr = data.data;
     ...
     * 请求代开房战绩列表
     */
    sendDaiList: function () {
        var postData = {
            'mid': cc.vv.userData.mid
        };
        let url = cc.vv.http.URL;
        cc.vv.http.sendRequest(url + "daikai_zhanji", postData, this.onReturnZhanjiList.bind(this));
    },
    ...
    onReturnZhanjiList: function (data) {
        ...
        if (data.status == 1) {
            ...
            this.list_data_0 = data.data;
            let prompt = this.view_arr[0].getChildByName("prompt");
            if(0 == data.data.data.length){
                prompt.active = true;
                for(let i = 0; i < this.content_arr[0].childrenCount; i++){
                    let node = this.content_arr[0].children[i];
                    if(node.active) node.active = false;
                }
                return
            }
            if(prompt.active) prompt.active = false;
            this.page_0 = Math.ceil(data.data.data.length / 10);
            if(0 == this.page_0) this.page_0 = 1;
            this.updateView(0);
        } else {
