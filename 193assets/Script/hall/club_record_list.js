...
cc.Class({
    ...
    updateView: function (data, index) {
        ...
        for (var i = 0; i < max_len; ++i) {
            ...
            if (len && i < len) {
                ...
                this.showItem(item, data[i], i + 1, index);
            } else {
                if (i < child_len) {
                    item = child_arr[i];
                    item.active = false;
    ...
    sendZhanjiList: function () {
        var postData = {
            "club_id": cc.clubControl.id,
            'page': 1,
            'mid': cc.vv.userData.mid
        };
        let url = cc.vv.http.URL;
        cc.vv.http.sendRequest(url + "club_game_record", postData, this.onReturnZhanjiList.bind(this));
    },
