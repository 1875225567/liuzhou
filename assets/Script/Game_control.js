...
cc.Class({
    ...
    onReturnJinru:function(data){
        ...
        //this.rule_lab.string = this.game_ren + "人  " + (this.roomInfo.guize.jushu + "局  ") +
        //    (this.roomInfo.guize.menqing.toString() == "1" ? "门清\n":"\n") +
        //    (this.roomInfo.guize.fangfei.toString() == "1" ? "房费均摊\n":this.roomInfo.guize.fangfei == 2 ? "房主支付\n":"俱乐部支付\n") +
        //    (this.roomInfo.guize.yu_num.toString() == "1" ? "爆炸鱼  " : "钓" + this.roomInfo.guize.yu_num + "鱼  ") +
        //    (this.roomInfo.guize.fengding.toString() == "0" ? "无限番\n":"自摸" + this.roomInfo.guize.fengding + "封顶\n") +
        //    (this.roomInfo.guize.yu_type == -1 ? "":this.roomInfo.guize.yu_type.toString() == "1" ? "一五九钓鱼\n":"跟庄钓鱼\n") +
        //    (this.roomInfo.guize.hua.toString() == "0" ? "" : "有花 ") + (this.roomInfo.guize.fenghu.toString() == "0" ? "" : "四笔封胡 ") +
        //    (this.roomInfo.guize.wufeng.toString() == "0" ? "" : "无风 ");
    ...
    detectIp:function(){
        ...
        cc.loadingControl.onToggleView('notice_layer', str);
        cc.log(arr,str);
    },
    ...
    onReturnZhuapai:function(data){
        ...
        if(data.mid.toString() == cc.vv.userData.mid.toString()){
            ...
            if(this.ting_hu && 0 < this.ting_hu.length){
                ...
                // this.game_layer.getChildByName("hu").width = 70 + this.ting_hu.length * 45;
                // for(var i = 0; i < this.ting_hu.length; i++){
                //     var hu_pai = cc.instantiate(this.hu_pai_prefab);
                //     hu_pai.position = cc.v2(i * 45, -5);
                //     hu_pai.parent = this.game_layer.getChildByName("hu").getChildByName("view");
                //     this.showMajIcon(hu_pai, this.ting_hu[i].pai, 0);
                //     hu_pai.getChildByName("num").getComponent(cc.Label).string = this.ting_hu[i].num + "张";
    ...
    showInteractive:function(data, dir, _dir){
        ...
        let skeletonData = "";
        //if(0 == data.msg) skeletonData = "meigui";
        //else if(1 == data.msg) skeletonData = "jidan";
        //else if(2 == data.msg) skeletonData = "feiwen";
        //else if(3 == data.msg) skeletonData = "tuoxie";
        //else if(4 == data.msg) skeletonData = "ganbei";
        //else if(5 == data.msg) skeletonData = "zhadan";
        ...
        cc.loader.loadRes("spine/hudongbiaoqing/" + skeletonData, sp.SkeletonData, function (err, spData) {
            var sp1 = user_hudong.getComponent(sp.Skeleton);
            var sp2 = other_hudong.getComponent(sp.Skeleton);
    ...
    doCountdownTime:function(){
        //每秒更新显示信息
        let num = 0, number = 0, tenNum = 0, res = 0;
        for(var i = 0; i < this.player_id_arr.length; i++){
            ...
            for(let i = 0; i < this.game_ren; i++){
                ...
                if(element.hasOwnProperty("z")){
                    ...
                    for(var j = 0; j < z.length; j++){
                        ...
                        if(count == 3){//碰
                            hand_arr.push({status:type, pai:z[j].pais, gang:0, dir:d});
                        }else if(count == 4){//杠   分明杠  暗杠
                            if( z[j].d_mid.toString() == ""){  //没有打牌mid信息即为暗杠
                                hand_arr.push({status:3, pai:z[j].pais, gang:1, dir:d});//暗杠
                            }else{
                                hand_arr.push({status:3, pai:z[j].pais, gang:0, dir:d});//明杠
                            }
                        }
                    }
                }
                ...
                if(element.hasOwnProperty("s")){
                    ...
                    if(i == 0){
                        ...
                        this.maj_hand_pai[i].sort(function(a,b){return a - b});//排序
                    }else{
                        if(s >= 14){
                            s = 14;
                        }
    ...
    changePlayerView:function(id, node, data, isShow, status, ready){
        ...
        if(data == null){
            ...
            for(let i in this.lixian){
                this.onLixian(this.lixian[i],true);
                if(data.id.toString() === this.lixian[i].toString()){
                    // cc.log("离线玩家有：",data.id.toString());
                    // node.getChildByName("user").getChildByName("offline").active = true;
                }
            }
